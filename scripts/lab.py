...
"""
    ...
    def build(self, jobs, modules):
        ...
        build_id = datetime.datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ...
        write_json(self.state / "last-full-build.json", {"status": "invalid",
                   "reason": "A build has started; a successful full build is required before launch.", "log": str(log)})
    ...
    def start(self, name, port, window=False):
        self.statuses(clean=True)
        if self.runtime_alive():
        ...
        if build.get("status") == "invalid":
            raise LabError(build["reason"])
        ...
        (runtime / "emulator-home").mkdir()
        (runtime / "avd-home").mkdir()
        ...
        command = [str(emulator), "-accel", "on", "-gpu", "swiftshader_indirect", "-no-audio",
    ...
    def verify(self, expect, timeout):
        ...
        text_output = self.adb("shell", "/product/bin/research-info").stdout
        text_fields = dict(line.split("=", 1) for line in text_output.splitlines())
        invalid = self.adb("shell", "/product/bin/research-info", "--invalid", check=False)
        debuggable = self.adb("shell", "getprop", "ro.debuggable").stdout.strip()
        fingerprint = self.adb("shell", "getprop", "ro.build.fingerprint").stdout.strip()
        ...
        help_result = self.adb("shell", "cmd", "activity", "help", check=False)
        help_text = help_result.stdout
        checks = {"help_response": help_result.returncode in (0, 255) and "Activity manager (activity) commands:" in help_text,
                  "text_matches_json": text_fields == info, "invalid_args": invalid.returncode == 2,
                  "debuggable_build": debuggable == "1", "android_release": info.get("android_release") == "13",
                  "build_id": bool(info.get("build_id")), "android_sdk": info.get("sdk") == TARGET["sdk"],
                  "product_flavor": info.get("build_flavor") == TARGET["product"] + "-" + TARGET["variant"],
                  "tool_version": info.get("tool_version") == "0.1.0",
                  "abi": info.get("abi") == "x86_64", "fingerprint": bool(fingerprint) and info.get("fingerprint") == fingerprint,
        ...
        result = {"time": now(), "expect": expect, "serial": meta["serial"], "info": info, "tool_text": text_output,
                  "core_output": core.stdout + core.stderr, "core_returncode": core.returncode, "help_returncode": help_result.returncode,
    ...
    @staticmethod
    def save_revision(source, bare, head, ref):
        # Git 2.34 receive-pack leaves its sideband pipe open when the
        # connectivity check returns early for an existing promisor tip.
        # Fetching into the local bare repository avoids that receiver path.
        # --no-filter is essential: every new private blob must be saved.
        if ref.startswith("refs/heads/snapshots/"):
            old = git(bare, "rev-parse", "--verify", ref, check=False)
            if old and old != head:
                raise LabError("A saved snapshot reference already points to a different commit.")
        run(["git", "-C", bare, "fetch", "--no-tags", "--no-filter", str(source), f"{head}:{ref}"], capture=True)
        if git(bare, "rev-parse", ref) != head:
    ...
    def snapshot(self, name, images=False):
        ...
        for key, path in (("product", self.product), ("frameworks-base", self.core)):
            ...
            self.save_revision(path, bare, head, f"refs/heads/snapshots/{name}")
            ...
            if key == "frameworks-base" and git(path, "branch", "--show-current") == TARGET["core_branch"]:
                self.save_revision(path, bare, head, f"refs/heads/{TARGET['core_branch']}")
        ...
        if runtime:
            ...
            if verification_file.exists():
                candidate = json.loads(verification_file.read_text())
                recorded = candidate.get("build", {})
                if recorded.get("core_commit") == commits["frameworks-base"]["commit"] and recorded.get("product_commit") == commits["product"]["commit"]:
        ...
        if images:
            ...
            paths += list(self.product_out.glob("*/build.prop"))
            paths += [self.product_out / name for name in ("system-qemu-config.txt", "VerifiedBootParams.textproto")]
            for source in paths:
                if source.is_file():
                    relative = source.relative_to(self.product_out)
                    target = image_dir / relative
                    target.parent.mkdir(parents=True, exist_ok=True)
                    run(["cp", "--reflink=auto", "--sparse=always", str(source), str(target)])
                    image_records[str(relative)] = {"size": target.stat().st_size, "sha256": protection.sha256(target)}
...
def main():
    ...
    p.add_argument("--window", action="store_true", help="Show the emulator desktop window")
    sub.add_parser("stop")
    ...
    try:
        ...
        with lab.lock():
            if args.command == "setup":
                ...
                lab.start(args.name, args.port, args.window)
